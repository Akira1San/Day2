...
entries = sg.apply_approximate(num_days=1, mode="early_fill")
print("=== Early fill day1 ===")
for e in entries:
    if e.start_minutes < 24*60:
        start_h = e.start_minutes // 60
        start_m = e.start_minutes % 60
        end_h = e.end_minutes // 60
        ...
        if "Movie collection" in e.video_name:
            ...
            marker = " [SERIES]"
        print(f"  {start_h:02d}:{start_m:02d} - {end_h:02d}:{end_m:02d}{marker} | {e.video_name}")
