...
class TagDialog(BaseTagDialog):
    ...
    def _create_blacklist_section(self):
        ...
        clear_sel_btn = QPushButton("Clear Selection")
        clear_sel_btn.clicked.connect(self.clear_blacklist_selection)
        btn_layout.addWidget(clear_sel_btn)
...
class SeriesConfigDialog(QDialog):
    ...
    def browse_collection(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Select Collection File", "", "JSON Files (*.json);;All Files (*)")
        if file_path:
...
class MultiSeriesDialog(BaseTagDialog):
    ...
    def refresh_series_list(self):
        ...
        for cfg in self.series_configs:
            ...
            display = f"{coll_name}: S{season}E{episode}, {count} eps, {mode}"
