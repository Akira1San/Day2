...
class GroupApproximateStrategy:
    """Strategy for group-approximate scheduling: all tags sorted by time as one group.

    Collects custom, series, and multi-series tags into a single sorted list
    by start time, places them sequentially so overlapping/adjacent windows
    are naturally grouped without gaps, then fills remaining time with random entries.
    """
    ...
    def generate(self, num_days: int = 1) -> List[ScheduleEntry]:
        ...
        for day_offset in range(num_days):
            ...
            for tag, abs_start, abs_end, duration in day_tags:
                ...
                scheduled_slots.append((slot_start, actual_end))
                current_pos = self.sg._consume_overlapping_tail(
                    slot_start, slot_end, current_pos, day_unused, random_entries,
                    used_random, final, day_offset,
                    min_end_threshold=slot_start,
                    scheduled_slots=scheduled_slots,
                )
